...
export async function GET(request: NextRequest) {
  ...
  const flatRows = (data ?? []).map((r) => ({
    ...
    nom: r.name,
    type: r.type,
    nature_realisation: r.realisation_nature ?? "",
    ...
    auteur_nature: r.author_type ?? "",
    date: r.date,
    statut: r.status,
    statut_validation: r.validation_status,
    beneficiaires_total: r.beneficiaries_total,
    beneficiaires_femmes: r.beneficiaries_female ?? "",
    beneficiaires_hommes: r.beneficiaries_male ?? "",
    photos: Array.isArray(r.photos) ? r.photos.join(" | ") : "",
    autres_infos: r.import_extras && typeof r.import_extras === "object" ? JSON.stringify(r.import_extras) : "",
    source: r.source,
